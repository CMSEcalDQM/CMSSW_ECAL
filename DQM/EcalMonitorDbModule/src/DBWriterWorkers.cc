...
*/
  ...
  */
    std::cout<<"\n Entered PedestalWriter::run"<<std::endl;
    bool result(true);
    ...
    for (std::map<int, unsigned>::iterator gainItr(gainToME_.begin()); gainItr != gainToME_.end(); ++gainItr) {
      ...
      for (MESet::const_iterator pItr(pedestalME.beginChannel(GetElectronicsMap())); pItr != pEnd; pItr.toNextChannel(GetElectronicsMap())) {
        ...
        //std::cout<<"pedestalME entries= "<<entries<<std::endl;
        if (entries < 1.)
          continue;
        ...
        int channelStatus(qItr->getBinContent());
        //std::cout<<"pedestal Channel Status= "<<channelStatus<<std::endl;
        bool channelBad(channelStatus == kBad || channelStatus == kMBad);
        if (channelBad)
    ...
    try {
      if (!pedestals.empty()){
        std::cout<<"\n **** Pedestals not empty! Inserting Pedestal\n";
        _db->insertDataArraySet(&pedestals, &_iov);}
      if (!pnPedestals.empty()){
        std::cout<<"****PN Pedestals not empty! Inserting PNpedestal\n";
  ...
  */
  ...
  TestPulseWriter::TestPulseWriter(edm::ParameterSet const &_ps)
    ...
    std::cout<<"\n Entered TestPulseWriter"<<std::endl;
  ...
  */
    std::cout<<"\n Entered TestPulseWriter::run"<<std::endl;
    ...
    try {
      if (!amplitude.empty()){
        std::cout<<"\n TestPulse Amp not empty. Inserting in DB"<<std::endl;
        _db->insertDataArraySet(&amplitude, &_iov); }
      if (!shape.empty()){
        std::cout<<"\n TestPulse Shape not empty. Inserting in DB"<<std::endl;
    ...
    std::cout<<"TestPulse result= "<<result<<std::endl;
    return result;
  }
