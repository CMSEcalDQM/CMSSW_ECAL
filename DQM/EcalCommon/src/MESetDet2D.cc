...
namespace ecaldqm
{
  ...
  void MESetDet2D::fill(
    ...
    unsigned iME(0);
    if(btype_ == binning::kPseudoStrip) iME = binning::findPlotIndex(edso.electronicsMap, otype_, _id.dccId(),binning::kPseudoStrip);
    else iME = binning::findPlotIndex(edso.electronicsMap, otype_, _id);
    checkME_(iME);
    ...
    if(btype_ == binning::kPseudoStrip){
      ...
      std::vector<DetId> ids(edso.electronicsMap->pseudoStripConstituents(stid.dccId(), stid.towerId(), stid.stripId()));
      unsigned nId(ids.size());
      for(unsigned iId(0); iId < nId; iId++){
    ...
    for(unsigned iME(0); iME < nME; iME++) {
      ...
      for(int ix(1); ix <= nbinsX; ix++){
        for(int iy(1); iy <= nbinsY; iy++){
          ...
          if(!binning::isValidIdBin(electronicsMap, obj, btype_, iME, bin)) continue;
